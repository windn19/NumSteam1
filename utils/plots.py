...
def save_one_box(event, xyxy, im, file=Path('im.jpg'), gain=1.02, pad=10, square=False, BGR=False, save=True,
                 cam_name='test_cam1', crop_coor=[], region=[]):
    ...
    if crop_coor[0] < int(xyxy[0, 0]) and crop_coor[1] < int(xyxy[0, 1]) \
        ...
        if event and region[0] < int(xyxy[0, 3]) < region[1]:
            ...
            UPLOAD_DIR_CROP = BASEDIR / 'images' / cam_name / 'crops'
            UPLOAD_DIR_IMAGE = BASEDIR / 'images' / cam_name / 'images'
            date = datetime.now().strftime('%d_%m_%Y_%H_%M_%S')
            f_crop = f'{date}_crop.jpg'
            log.info(f'Камера {cam_name}: Номер сохранен под именем: {f_crop}')
            try:
